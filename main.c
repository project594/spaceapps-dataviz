...
double gaussian_odd(double x, double A, double mu, double sigma){
	double v = A * exp(-((x-mu)*(x-mu)) / (2 * sigma * sigma));
	if (x >= mu){
		return A;
	}
	return v;
...
void graphCurve(planet* p, graph* g){
    ...
    for (int i = 0; i < g->size; i++) {
        ...
        DrawPixel(i + g->xpos, (int)(72*a) + g->ypos, WHITE);
    }	
}

void drawSpectrum(spectrum* spectrum){
	for (int i = 0; i < spectrum->width; i++) {
	 	double h = gaussian_odd((double)(i * 1.0 / spectrum->width * 1.0), 0.8, 0.6, 0.1) * 360.0;
		DrawLine(i + spectrum->xpos,spectrum->ypos,i + spectrum->xpos,spectrum->ypos + spectrum->height + spectrum->ypos, ColorFromHSV(h,1.0, 0.1 + gaussian((double)(i * 1.0 / spectrum->width * 1.0), 0.8, 0.5, 0.15)));
    		DrawRectangle(spectrum->xpos, spectrum->ypos, spectrum->width, 50, BLACK);
		DrawRectangle(spectrum->xpos, spectrum->ypos + spectrum->height - 50, spectrum->width, 50, BLACK);
		DrawRectangleLines(spectrum->xpos, spectrum->ypos, spectrum->width, spectrum->height, GRAY);
	}
}
...
void UpdateDrawFrame(void)
{
	...
	drawSpectrum(&g_spectrum);
	//DrawLine(0,0,500,500,RED);
        graphCurve(&g_planet, &g_graph);
        DrawFPS(10, 10);
	DrawText(TextFormat("score: %02.09f ", g_koi_score), 10, 70, 20, GRAY);
...
{   
    ...
    g_spectrum.width = GetScreenWidth() - (int)(150 * M_PI);
    g_spectrum.height = 150;
    g_spectrum.xpos = (int)(150 * M_PI);
    g_spectrum.ypos = GetScreenHeight() - 150;
